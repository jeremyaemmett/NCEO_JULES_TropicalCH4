...
def kfs(x0_t, param, f, t, tobs, y, H, R, rho, ne, met,
    ...
    if back0 == "fixed":
        ...
        Xb[..., 0] = rng.normal(scale=desv, size=(nx, ne))
        Xb[..., 0] = Xb[..., 0] - np.mean(Xb[..., 0], axis=-1, keepdims=True)
        if param_estimate:
            Param_a[..., 0] = rng.normal(scale=desv, size=(Nparam, ne))
            Param_a[..., 0] = Param_a[..., 0] - \
    ...
    for j in range(len(tobs) - 1):
        ...
        X[:nx] = Xb[..., (j + 1) * o2t]
        if param_estimate:
            X[nx:] = paramold + alpha*rng.standard_normal((Nparam,ne))

        # The assimilation
        X, rho = enkfs(X, y[:, j + 1], Hall, R, rho, met,
                       localDomainObsMask, locmatrix, adaptive, rng)
        Xa[..., (j + 1) * o2t] = X[:nx]
        if param_estimate:
            Param_a[..., j+1] = X[nx:]
...
##############################################################################
## The EnKF algorithms
def enkfs(Xb, y, H, R, rho, met, localDomainObsMask, locmatrix, adaptive, rng):
    ...
    rng : np.random.Generator
         random number generator for perturbed obs and adaptive inflation
    ...
    if met=='SEnKF':
        ...
        Xa.fill(np.nan)
        for jm in range(ne):
            yaux = y + sqR@rng.standard_normal((ny, 1))
...
def getlocmat(nx, ny, H, lam, loctype):
    ...
    # If we want something smooth, we use the Gaspari-Cohn function
        ...
        raise NotImplementedError(f'loctype {loctype} not implemented, try GC or cutoff')
...
if __name__ == '__main__':
    ...
    rng_m = np.random.default_rng(seed=10)
    nx_m = 12
    F = 8
    x0 = rng_m.random(nx_m)
    tf_m = 4
    deltat_m = 0.025
    #Create truth.
    xt = lorenz96(x0, tf_m, deltat_m, 0, 8.0)
    t_m = createTime(0., tf_m, deltat_m, 0)
    _, H_m = createH('1010', nx_m)
    tobs_m, y_m, R_m = gen_obs(t_m, xt, 2, H_m, 2.0, 1)
    Xb_m, xb_m, Xa_m, xa_m, L_obs_m, L_x_m = kfs(x0, 8.0,
                                                 lorenz96, t_m, tobs_m, y_m,
                                                 H_m, R_m, 0., 24,
