...
def func(args):
    ...
    cmap1 = "PuOr" # default: "plasma" 
    cmap2 = "BrBG" # default: "RdYlBu_r"
    ...
    for variable in variables:
        ...
        for iplot in range(0, len(filepaths)):
            ...
            for level, ax in zip(levels, axs[::-1]):
                # Plot variable
                if args.basefilepath is None:
                    im = ax.contourf(lon_coord, lat_coord, fields_plot[iplot][level], cmap=cmap1, levels=clevels[level])
                else:
                    im = ax.contourf(lon_coord, lat_coord, fields_plot[iplot][level], cmap=cmap2, levels=clevels[level])
            ...
            # Added by GH for DA training course in Reading 
            if args.basefilepath:
                diff = field - field_base 
                rmse = np.sqrt(np.mean(diff**2))
                axs[0].text(0.01, 1.15, f"RMSE: {rmse:.6e}", transform=axs[0].transAxes,
